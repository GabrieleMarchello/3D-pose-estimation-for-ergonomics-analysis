...
try:
    while True:
        ...
        bparts=TfPoseEstimator.draw_humans_3d(fig, ax, depth_image, depth_scale, rs, camera_info, humans, [], imgcopy=False)
        ...
        img_out = TfPoseEstimator.draw_humans(color_image, humans, bparts, imgcopy=False)
        cv2.putText(img_out,
                    "FPS: %f" % (1.0 / (time.time() - fps_time)),
                    (10, 10),  cv2.FONT_HERSHEY_SIMPLEX, 0.5,
                    (0, 255, 0), 2)
        
        cv2.imshow('tf-pose-estimation result', img_out)
        fps_time = time.time()
